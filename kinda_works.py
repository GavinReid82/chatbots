...
class AlienBot:
    ...
    def describe_planet_intent(self):
        ...
        if not hasattr(self, 'used_responses'):
            self.used_responses = []  # Track indices of responses that have been given
        
        if len(self.used_responses) == len(responses):
            return "That's all I feel comfortable telling you."
        
        available_responses = [i for i in range(len(responses)) if i not in self.used_responses]
        selected_index = random.choice(available_responses)
        
        self.used_responses.append(selected_index)
